          ...
          {group.parts.map((part, j) => {
              ...
              return (
                ...
                >
                  <text attributes={TextAttributes.DIM} fg={colors.muted}>
                    <em fg={colors.thinking}>REASONING</em>  {part.text}
                  </text>
                </box>
              );
            }
            ...
            if (isToolPart(part)) {
              ...
              return (
                ...
                >
                  <text attributes={TextAttributes.DIM} fg={colors.muted}>
                    <em fg={colors.info}>{formatToolName(toolName).toUpperCase()}</em>  {formatToolArgs(part)}
            ...
            if (part.type === "text") {
              return (
                <box key={`text-${j}`} paddingX={3} width="100%">
                  <text fg={colors.foreground}>{part.text}</text>
                </box>
              );
            }
      ...
      <box paddingX={3} paddingY={1} gap={1} width="100%">
        <box flexDirection="row" gap={2}>
          ...
          <box flexDirection="row" gap={1}>
            <text fg={colors.foreground ?? colors.primary}>
              AUTOCRAFT CODE
            </text>
